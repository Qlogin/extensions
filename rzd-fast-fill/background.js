...
function fillContextMenu(contextData) {
    chrome.contextMenus.removeAll(function() {
        if (chrome.extension.lastError) {
            console.log("Unable to remove all from context menu: " + chrome.extension.lastError.message);
        }
    });
    ...
    contextData.forEach(function(pass) {
        ...
        chrome.contextMenus.create({
            ...
            function() {
                if (chrome.extension.lastError) {
                    console.log("Unable to add passenger to context menu: " + chrome.extension.lastError.message);
                }
        });
    });
    ...
    chrome.contextMenus.create({
        id:       "PASS_ADD",
        title:    "Сохранить пассажира",
        contexts: ["editable"]},
        function() {
            if (chrome.extension.lastError) {
              console.log("Unable to add passenger save to context menu: " + chrome.extension.lastError.message);
            }
    });
    ...
    if (contextData.length > 0) {
        chrome.contextMenus.create({
            ...
            function() {
                if (chrome.extension.lastError) {
                    console.log("Unable to add passenger remove to context menu: " + chrome.extension.lastError.message);
                }
        });
    }
    ...
    contextData.forEach(function(pass) {
        ...
        chrome.contextMenus.create({
            id:       "PASS_REMOVE" + userId,
            parentId: "PASS_REMOVE",
            title:    firstName + " " + lastName,
            contexts: ["editable"]},
            function() {
                if (chrome.extension.lastError) {
                    console.log("Unable to add passenger remove item to context menu: " + chrome.extension.lastError.message);
                }
        });
    });
}
...
function contextMenuClick(info) {
    if (info.menuItemId.indexOf("PASS_LOAD") == 0) {
        ...
        chrome.contextMenus.remove(info.menuItemId, function() {
            if (chrome.extension.lastError) {
                console.log("Unable to remove item from context menu: " + chrome.extension.lastError.message);
            }
        });
